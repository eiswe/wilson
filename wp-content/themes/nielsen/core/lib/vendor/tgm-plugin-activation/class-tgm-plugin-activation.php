...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->_marketplace = YIT_MARKETPLACE == 'tf' ? 'ThemeForest' : 'Yithemes'; // YIT

            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'yit' ),
                'menu_title'                     => __( 'Install Plugins', 'yit' ),
                'installing'                     => __( 'Installing Plugin: %s', 'yit' ),
                'oops'                           => __( 'Something went wrong.', 'yit' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'yit' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'yit' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'yit' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'yit' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'yit' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'yit' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'yit' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'yit' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'yit' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'yit' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'yit' ),
                'dashboard'                      => __( 'Return to the dashboard', 'yit' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'yit' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'yit' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'yit' ),
                'dismiss'                        => __( 'Dismiss this notice', 'yit' ),
            );
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <div class="update-nag">
                    <?php $msg = sprintf(__('Some Pre-Packaged plugins <strong>must be moved</strong> from %s package to theme/plugins directory, inside the theme folder, <strong>before install</strong> plugins from this page.', 'yit'), $this->_marketplace); ?>
                    <?php echo $msg; ?>
                </div>
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'yit' ) . '">' . __( 'Return to the Dashboard', 'yit' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'yit' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'yit' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'yit' ) . '</a>'  : '';
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'yit' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'yit' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'yit' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'yit' ) : __( 'Recommended', 'yit' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'yit' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'yit' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                ...
                if ( 'Pre-Packaged' == $item['source'] && ! file_exists( $item['url'] ) ) {
                    $msg = sprintf(__('Get the ZIP package of the plugin from %s package, put it inside theme/plugins folder of the theme and reload this page', 'yit'), $this->_marketplace );
                    $actions = array(
                        'install' => '<span style="font-size: 12px; color: #aaa;">' . $msg . '</span>',
                    );
                } else {

                    $actions = array(
                        'install' => sprintf(
                            '<a href="%1$s" title="' . esc_attr__( 'Install', 'yit' ) . ' %2$s">' . __( 'Install', 'yit' ) . '</a>',
                            esc_url(
                                wp_nonce_url(
                                    add_query_arg(
                                        array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'yit' ) . ' %2$s">' . __( 'Activate', 'yit' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'yit' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'yit' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'yit' ), self_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'yit' ),
                'source' => __( 'Source', 'yit' ),
                'type'   => __( 'Type', 'yit' ),
                'status' => __( 'Status', 'yit' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'yit' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'yit' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( TGM_Plugin_Activation::$instance->menu ) ), self_admin_url( 'themes.php' ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'yit' ), $imploded );
                }
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'yit' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'yit' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'yit' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'yit' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'yit' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'yit' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'yit' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'yit' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'yit' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'yit' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'yit' ) . '</span><span class="hidden">' . __( 'Hide Details', 'yit' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'yit' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'yit' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'yit' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'yit' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'yit' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'yit' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'yit' ) . '</span><span class="hidden">' . __( 'Hide Details', 'yit' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'yit' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'yit' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                            if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( TGM_Plugin_Activation::$instance->menu ), self_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                                $complete[] = $plugin;
                                break;
                            }
                            // Nothing to store.
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'yit' ) . '">' . __( 'Return to the Dashboard', 'yit' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
