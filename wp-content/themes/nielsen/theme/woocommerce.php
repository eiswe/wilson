...
function yit_woocommerce_hooks() {
    ...
    if ( yit_get_option( 'shop-buy-now-button' ) == 'yes' ) {
        add_filter( yit_get_add_to_cart_redirect_filter_name(), 'yit_redirect_to_checkout' );
    }
    ...
    if ( version_compare( preg_replace( '/-beta-([0-9]+)/', '', WC()->version ), '2.5', '>=' ) ) {
        remove_action( 'woocommerce_shop_loop_subcategory_title', 'woocommerce_template_loop_category_title', 10 );
        add_action( 'woocommerce_after_subcategory_title' , 'yit_woocommerce_after_subcategory_title' , 10 , 2 );
    }
    ...
    if ( yit_get_option( 'shop-product-price' ) == 'no' ) {
    ...
    if ( yit_get_option( 'shop-quick-view-enable' ) == 'yes' ) {
        add_action( 'woocommerce_before_shop_loop_item_title', 'yit_add_quick_view_button', 20 );
    }
    ...
    if ( yit_get_option( 'shop-enable' ) == 'no' || yit_get_option( 'shop-single-product-price' ) == 'no' ) {
    ...
    if ( is_quick_view() ) {
        add_filter( 'woocommerce_single_product_image_html', 'yit_product_image_slider_quick_view' );
    }
 ...
 */
if( ! function_exists('yit_woocommerce_after_subcategory_title') ) {

    function yit_woocommerce_after_subcategory_title( $category , $show_counter = 1 ) {
        ?>
...
if ( !function_exists( 'yit_get_current_cart_info' ) ) {
    ...
    function yit_get_current_cart_info() {
        ...
        return array(
            sprintf( _n( '%s item', '%s items', $items, 'yit' ), $items ),
            $total
        );
    }
}
    ...
    function yit_woocommerce_add_tabs( $tabs = array() ) {
        ...
        if ( !empty( $custom_tabs ) ) {
            ...
            foreach ( $custom_tabs as $tab ) {
                ...
                yit_wpml_register_string( 'nielsen-theme' , 'custom_tab_'.sanitize_title( $tab["name"] ) , $tab["name"] );
                $tab["name"] = yit_wpml_string_translate( 'nielsen-theme' , 'custom_tab_'.sanitize_title( $tab["name"] ) , $tab["name"] );
                yit_wpml_register_string( 'nielsen-theme' , 'custom_tab_'.sanitize_title( $tab["value"] ) , $tab["value"] );
                $tab["value"] = yit_wpml_string_translate( 'nielsen-theme' , 'custom_tab_'.sanitize_title( $tab["value"] ) , $tab["value"] );
...
add_action( 'yit_move_woocommerce_js' , 'yit_do_move_woocommerce_js' );

function yit_do_move_woocommerce_js() {
    ...
    function yit_add_to_cart_success_ajax( $datas ) {
        ...
        <div class="added_to_cart">
            ...
            <?php if ( isset( $_REQUEST['product_id'] ) ):
                ...
                $id      = ( isset( $_REQUEST['variation_id'] ) && $_REQUEST['variation_id'] != 'false' ) ? $_REQUEST['variation_id'] : $_REQUEST['product_id'];
...
if ( !function_exists( 'yit_woocommerce_object' ) ) {
    ...
    function yit_woocommerce_object() {
        ...
        wp_localize_script( 'jquery', 'yit_woocommerce', array(
            ...
            'ajax_search_premium_installed'    => defined( 'YITH_WCAS_PREMIUM' ),
        ) );
...
if ( ! function_exists( 'yit_wc_sanitize_html' ) ) {
    ...
    function yit_wc_sanitize_html( $sanitized, $class ) {
        ...
        if( $class == 'view' ) {
            return $sanitized.' btn btn-flat-red ';
        }
