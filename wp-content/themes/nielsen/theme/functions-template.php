...
if ( !function_exists( 'yit_back_to_top' ) ) {
    ...
    function yit_back_to_top() {
        if ( yit_get_option('general-show-back-to-top') == 'yes' ) {
            echo '<div id="back-top"><a href="#top"><i class="fa fa-chevron-up"></i>' . apply_filters( 'yit_back_to_top_text' , __('Back to top', 'yit') )  . '</a></div>';
        }
    }
}
...
if( ! function_exists('yit_page_title') ){
    ...
    function yit_page_title(){
        ...
        if ( is_archive() ) {
            ...
            elseif ( is_category() ) {
                return sprintf( __( 'Category: %s', 'yit' ), single_cat_title( '', false ) );
            }
...
if ( ! function_exists( "yit_get_contact_info_item_custom" ) ){
    ...
    function yit_get_contact_info_item_custom( $icon_type,  $name_icon, $name_icon_theme, $text, $value, $email_link = '' ){
        if ( isset( $value ) && $value != '' ){
            ...
            $show_icon = isset( $name_icon ) && ( $name_icon != '' && $name_icon != 'None' );
            ?>
            <li>
                <?php if ( $show_icon && $icon_type=='custom'  ) :  ?>
                    <div class="<?php echo esc_attr( $container_class ) ?>" style="background-image:url(<?php echo $name_icon ?>)"></div>
                    <?php elseif ( $show_icon && $icon_type=='theme-icon'  ) :  ?>
                    <div class="<?php echo esc_attr( $container_class ) ?> theme-icon"><?php echo '<i class="fa fa-'.$name_icon_theme.'"></i>'?></div>
                <?php endif ?>
...
if ( ! function_exists( 'yit_move_woocommerce_js' ) ) {
    function yit_move_woocommerce_js() {
        do_action( 'yit_move_woocommerce_js' );
    }
}
...
if ( !function_exists( 'yit_team_add_fields' ) ) {
    function yit_team_add_fields() {
        return array(
            ...
            'linkedin'   => array(
                'label' => __( 'Linkedin', 'yit' ),
                'type'  => 'text',
                'desc'  => __( 'Insert linkedin address', 'yit' ),
                'std'   => ''
            ),
        );
    }
